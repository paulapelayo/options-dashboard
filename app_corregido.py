...
def _cos_call_raw(S0, K_array, r, q, tau, v0, kappa, theta, xi, rho, N=512, L=20):
    """COS con ventana de truncamiento DESPLAZADA POR STRIKE (Fang-Oosterlee 2008,
    seccion de precios multi-strike) -- CORREGIDO.

    BUG ENCONTRADO (comparando contra quant_options_dashboard5.zip y verificado
    numericamente contra integracion directa de Fourier, sin este fix): la version
    anterior fijaba la ventana [a,b] SOLO con los cumulantes del modelo (sin
    desplazarla por x0=log(S0/K)) y metia el strike unicamente como fase compleja.
    Eso es valido cerca del dinero, pero para contratos muy OTM/ITM combinados con
    tau ultra-corto (vencimientos de horas/pocos dias) el error medido era enorme:
    un call que valia $0.00 de verdad llegaba a mostrar $24.57, $29.97 o hasta
    $122.23 segun el caso (verificado contra referencia por integracion directa).
    Peor aun: los guardrails de heston_calls_fast (no-arbitraje, monotonia en K) NO
    detectan esto cuando se precia un SOLO strike (la prueba de monotonia necesita
    al menos 2 strikes para comparar) -- exactamente el caso de uso del Panel 1 al
    pedir un contrato especifico.

    El ancho de la ventana (b-a) NO depende del strike (solo de los cumulantes del
    modelo), asi que la funcion caracteristica se evalua UNA sola vez y se reusa
    para todos los strikes; solo los coeficientes de payoff (chi/psi), que si
    dependen de la posicion absoluta de la ventana desplazada, se recalculan por
    strike -- mucho mas barato que una cuadratura adaptativa por strike.

    Devuelve (precios, c2); precios=None si c2<=0 (fórmula de cumulantes rota)."""
    ...
    width = 2 * L * np.sqrt(c2)  # ancho fijo: no depende del strike
    k = np.arange(N)
    kpi = k * np.pi / width
    u_cf = np.where(kpi == 0, 1e-8, kpi)
    cf0 = heston_cf_j(u_cf, 2, 0.0, v0, tau, r, q, kappa, theta, xi, rho)  # 1 sola evaluacion

    X = np.log(S0 / K_array)  # x0 por strike
    prices = np.empty_like(K_array)
    for i, x0 in enumerate(X):
        a = x0 + c1 - L * np.sqrt(c2)
        b = x0 + c1 + L * np.sqrt(c2)
        c_eff = min(max(a, 0.0), b)
        chi = (1 / (1 + kpi**2)) * (
            np.cos(kpi * (b - a)) * np.exp(b) - np.cos(kpi * (c_eff - a)) * np.exp(c_eff)
            + kpi * np.sin(kpi * (b - a)) * np.exp(b) - kpi * np.sin(kpi * (c_eff - a)) * np.exp(c_eff)
        )
        psi = np.where(
            k == 0, b - c_eff,
            (np.sin(kpi * (b - a)) - np.sin(kpi * (c_eff - a))) * width / (k * np.pi + 1e-30),
        )
        Uk = 2 / width * (chi - psi)
        phase = np.exp(1j * kpi * (x0 - a))
        terms = np.real(phase * cf0) * Uk
        terms[0] *= 0.5
        prices[i] = K_array[i] * np.exp(-r * tau) * terms.sum()
    return prices, c2
...
def calibrate_heston(market_rows, S0, r, q, n_candidatos=30, n_refinar=10, seed=1,
    ...
    converja en un ajuste no lineal de 5 parametros. Se sube a 30 candidatos LHS,
    10 refinados, max_nfev=200 (mismo presupuesto validado en el resto del proyecto).
...
# ============================================================
def heston_mc_qe(S0, K, tau, r, q, v0, kappa, theta, xi, rho,
                  option_type="call", n_paths=20_000, n_steps=80, seed=0, psi_c=1.5):
    """Monte Carlo con el esquema Quadratic-Exponential de Andersen (2008): simula
    la SDE completa de Heston sin absorcion/reflexion (evita varianza negativa POR
    CONSTRUCCION, a diferencia de esos dos parches que sesgan el resultado). Sirve
    como prueba de fuego INDEPENDIENTE del pricer analitico/COS: si el precio
    semi-analitico no cae dentro del intervalo de confianza de este Monte Carlo,
    hay un bug en la implementacion de Fourier/COS -- no en la simulacion.
    Devuelve (precio, error_estandar) para construir el IC 95%: precio ± 1.96*se."""
    rng = np.random.default_rng(seed)
    dt = tau / n_steps
    ...
    for _ in range(n_steps):
        ...
        v_new[qm] = a_ * (np.sqrt(np.maximum(b2, 0)) + Z) ** 2
        ...
        v_int = 0.5 * (v + v_new) * dt
        lnS = (lnS + (r - q) * dt - 0.5 * v_int
    ...
    if option_type == "call":
        ...
        payoff = np.maximum(K - ST, 0) * np.exp(-r * tau)
    return float(payoff.mean()), float(payoff.std() / np.sqrt(n_paths))


def kappa_xi_loss_grid(rows, S0, r, q, v0_fijo, theta_fijo, rho_fijo,
                        kappa_range=(0.3, 5.0), xi_range=(0.1, 1.4), n=18):
    """Barre kappa y xi en una malla, fijando v0/theta/rho en sus valores YA
    calibrados, y evalua la perdida ponderada en cada punto con el motor COS rapido
    (heston_prices_fast). Reproduce el diagnostico del notebook 5: la perdida suele
    ser casi plana a lo largo de una CURVA en (kappa,xi) -- evidencia visual de que
    los datos no fijan kappa y xi por separado, solo alguna combinacion de ambos.
    Devuelve (kappas, xis, L) con L en escala log10 para que el valle sea visible
    en un heatmap/contour."""
    kappas = np.linspace(*kappa_range, n)
    xis = np.linspace(*xi_range, n)
    L = np.zeros((n, n))
    for a, kap in enumerate(kappas):
        for b, x in enumerate(xis):
            total = 0.0
            for K, tau_, mkt, spread, opt in rows:
                try:
                    mdl = heston_prices_fast(S0, np.array([K]), r, q, tau_,
                                              v0_fijo, kap, theta_fijo, x, rho_fijo, opt)[0]
                    w = 1.0 / max(spread, 0.01) ** 2
                    total += w * (mdl - mkt) ** 2
                except Exception:
                    total += 1e6
            L[a, b] = total
    return kappas, xis, np.log10(L + 1e-12)


def heston_calibration_report_vol_points(rows, S0, r, q, params):
    """Valida el ajuste en PUNTOS DE VOLATILIDAD (no solo en precio) -- la unidad en
    la que de verdad se juzga un ajuste de smile en mesa: un error de $0.01 puede ser
    enorme o insignificante en vol dependiendo de donde este el strike; en vol points
    la escala es comparable entre strikes. También reporta sensatez económica
    (rho<0, leverage effect esperado en equities) y Feller."""
    v0_, theta_, kappa_, xi_, rho_ = params
    iv_errors = []
    for K, tau_, mkt, spread, opt in rows:
        iv_mkt = bs_implied_vol(mkt, S0, K, r, q, tau_, opt)
        mdl_price = heston_price(S0, K, r, q, tau_, v0_, kappa_, theta_, xi_, rho_, opt)
        iv_mdl = bs_implied_vol(mdl_price, S0, K, r, q, tau_, opt)
        if np.isnan(iv_mkt) or np.isnan(iv_mdl):
            continue
        iv_errors.append((iv_mdl - iv_mkt) * 100.0)
    iv_errors = np.array(iv_errors)
    rmse_iv = float(np.sqrt(np.mean(iv_errors**2))) if len(iv_errors) else np.nan
    max_err_iv = float(np.max(np.abs(iv_errors))) if len(iv_errors) else np.nan
    feller_ok, feller_lhs, feller_rhs = feller_condition(params)
    return dict(
        rmse_vol_points=rmse_iv, max_error_vol_points=max_err_iv,
        n_puntos_validos=len(iv_errors), rho_negativo=rho_ < 0,
        feller_ok=feller_ok, feller_lhs=feller_lhs, feller_rhs=feller_rhs,
    )
...
def get_clean_chain(ticker, expiry):
    ...
    CHAIN_COLUMNS = [
        ...
        "openInterest", "impliedVolatility", "lastPrice", "snapshot",
    ]
    if not rows:
...
def calibrate_for_expiry(ticker, expiry, S0, r, q, kappa_fijo=None):
    ...
    return params, fit_obj, len(rows), calib_warning, rows


with st.spinner("Calibrando Heston a la sonrisa de este vencimiento..."):
    heston_params, fit_obj, n_quotes_calib, calib_warning, rows_calib = calibrate_for_expiry(
...
def build_smile_data(chain_df, S0, r, q, tau):
    ...
    sort_values('moneyness') de más abajo truena con KeyError en vez de fallar con
...
# ============================================================
# 14. APARTADO EXTRA — VALIDACIONES INDEPENDIENTES DEL MOTOR DE HESTON
# (adoptado de la revisión cruzada contra quant_options_dashboard5.zip)
# ============================================================
st.markdown("---")
st.markdown("### Extra · Validaciones independientes de Heston")
st.caption(
    "Tres chequeos que no dependen del propio pricer analítico/COS, para poder "
    "defender frente a la mesa que el motor está bien y no solo 'se ve razonable'."
)

with st.expander("🎲 Validación cruzada: Monte Carlo QE (Andersen 2008)"):
    st.caption(
        f"Simula la SDE completa de Heston (20,000 trayectorias, esquema Quadratic-Exponential "
        f"que evita varianza negativa por construcción) para el contrato seleccionado "
        f"({ticker} {option_type.upper()} K={strike:g}, exp {expiry}) y lo compara contra el "
        f"precio semi-analítico. Si no coinciden dentro del intervalo de confianza, hay un bug "
        f"en la implementación de Fourier/COS — no en la simulación."
    )
    if st.button("Correr Monte Carlo QE"):
        with st.spinner("Simulando 20,000 trayectorias..."):
            mc_price, mc_se = heston_mc_qe(S0, strike, tau, r, q, v0, kappa, theta, xi, rho, option_type)
        ci_lo, ci_hi = mc_price - 1.96 * mc_se, mc_price + 1.96 * mc_se
        dentro = ci_lo <= price_heston <= ci_hi
        c1, c2 = st.columns(2)
        c1.metric("Heston analítico/COS", f"${price_heston:,.4f}")
        c2.metric("Monte Carlo QE", f"${mc_price:,.4f}", f"± {1.96*mc_se:.4f} (IC 95%)")
        if dentro:
            st.success(
                f"✅ El precio analítico (${price_heston:,.4f}) cae dentro del IC 95% de Monte Carlo "
                f"[{ci_lo:,.4f}, {ci_hi:,.4f}] — el pricer de Fourier/COS está validado de forma "
                f"independiente para este contrato."
            )
        else:
            st.warning(
                f"⚠️ El precio analítico (${price_heston:,.4f}) NO cae dentro del IC 95% de Monte Carlo "
                f"[{ci_lo:,.4f}, {ci_hi:,.4f}]. Con τ muy corto esto puede ser ruido de simulación "
                f"(sube n_paths) o señal real de un problema numérico — no lo ignores antes de la defensa."
            )

with st.expander("🗺️ Diagnóstico de identificabilidad κ–ξ (notebook 5)"):
    st.caption(
        "Barre κ y ξ en una malla fijando v₀, θ, ρ en sus valores calibrados, y colorea la pérdida "
        "(escala log10). Si ves una CURVA de valores bajos en vez de un solo mínimo aislado, significa "
        "que el mercado no fija κ y ξ por separado — solo alguna combinación de ambos. Es el problema "
        "de identificabilidad que motivó la opción 'fijar κ' en el sidebar."
    )
    if st.button("Calcular malla κ–ξ (puede tardar ~15-30s)"):
        with st.spinner("Evaluando la pérdida en la malla κ–ξ..."):
            kappas_grid, xis_grid, L_grid = kappa_xi_loss_grid(rows_calib, S0, r, q, v0, theta, rho)
        fig_kxi = go.Figure(data=go.Contour(
            z=L_grid, x=xis_grid, y=kappas_grid,
            colorscale="Viridis", colorbar=dict(title="log10(pérdida)"),
        ))
        fig_kxi.add_trace(go.Scatter(
            x=[xi], y=[kappa], mode="markers",
            marker=dict(color="red", size=12, symbol="x"), name="Calibrado",
        ))
        fig_kxi.update_layout(
            template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
            height=450, margin=dict(l=10, r=10, t=30, b=10),
            xaxis_title="ξ (vol-of-vol)", yaxis_title="κ (reversión)",
        )
        st.plotly_chart(fig_kxi, use_container_width=True)

with st.expander("📐 Validación de la calibración en puntos de volatilidad"):
    st.caption(
        "El RMSE en dólares puede ser engañoso: $0.01 de error es enorme en un contrato barato e "
        "insignificante en uno caro. En puntos de volatilidad implícita la escala es comparable "
        "entre strikes — es la unidad en la que de verdad se juzga un ajuste de smile en mesa."
    )
    report = heston_calibration_report_vol_points(rows_calib, S0, r, q, heston_params)
    rc1, rc2, rc3 = st.columns(3)
    rc1.metric("RMSE (vol points)", f"{report['rmse_vol_points']:.2f}" if np.isfinite(report['rmse_vol_points']) else "N/A")
    rc2.metric("Error máximo (vol points)", f"{report['max_error_vol_points']:.2f}" if np.isfinite(report['max_error_vol_points']) else "N/A")
    rc3.metric("Puntos válidos", report["n_puntos_validos"])
    if report["rho_negativo"]:
